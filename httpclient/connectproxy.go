...
// Direct use:
//     /* Make a proxy.Dialer */
//     d, err := connectproxy.NewConnectproxy("https://proxyserver:4433", proxy.Direct)
//     if nil != err{
//             panic(err)
//     }
//
...
//     proxy.RegisterDialerType("http", connectproxy.NewConnectproxy)
//     proxy.RegisterDialerType("https", connectproxy.NewConnectproxy)
//
...
func RegisterDialerFromURL(registerHTTP, registerHTTPS bool) {
	if registerHTTP {
		proxy.RegisterDialerType("http", NewConnectproxy)
	}
	if registerHTTPS {
		proxy.RegisterDialerType("https", NewConnectproxy)
	}
}
...
// NewConnectproxy returns a proxy.Dialer given a URL specification and an underlying
// proxy.Dialer for it to make network requests.  NewConnectproxy may be passed to
